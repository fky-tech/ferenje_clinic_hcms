...
import { UserPlus, CreditCard, MapPin, Calendar, Activity, Hash, DollarSign, Save, X, ChevronRight, Phone, Home, User } from 'lucide-react';
        ...
        <div className="w-full">
            ...
            <div className="relative">
                ...
                <input
                    ...
                    ${icon ? 'pl-9' : ''}
                    ${props.disabled ? 'opacity-60 cursor-not-allowed bg-gray-100' : 'bg-white'}
                    ${className}`}
                />
            </div>
...
export default function RegisterPatient() {
        ...
        return (
            <div className="w-full">
                ...
                <div className="relative">
                    ...
                    <input
                        {...inputProps}
                        className={`w-full bg-gray-50 border ${error ? 'border-red-300 focus:ring-red-200' : 'border-gray-200 focus:border-blue-500 focus:ring-blue-500/20'} text-gray-900 text-sm rounded-lg px-3 py-2.5 outline-none transition-all placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 ${icon ? 'pl-9' : ''} ${props.disabled ? 'opacity-60 cursor-not-allowed bg-gray-100' : 'bg-white'} ${className}`}
                        autoComplete="off"
    ...
    return (
        <div className="min-h-screen bg-gray-50/50  flex flex-col mt-[-26px] font-sans">
            ...
            <form onSubmit={handleSubmit} className="w-full max-w-[1300px]sm:rounded-2xl sm:border-white overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[90vh]">
                ...
                <div className="bg-white border-b border-gray-100 px-4 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0 shrink-0">
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        <div className="flex-1 sm:flex-none">
                            <h1 className="text-lg sm:text-xl font-bold text-gray-900 leading-tight">Patient Registration</h1>
                            <p className="text-xs text-gray-500 font-medium mt-0.5">New Admission & Card Issuance</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-full border border-gray-100 self-end sm:self-center">
                        <span className="text-xs text-gray-400 font-medium uppercase tracking-wider">ID</span>
                        <span className="text-sm font-mono font-bold text-gray-700">#{formData.patient_id}</span>
                    </div>
                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            <select
                                                ...
                                                value={formData.Sex}
                                                onChange={handleChange}
                                                className={`w-full px-3 py-2.5 bg-gray-50 border ${errors.Sex ? 'border-red-300' : 'border-gray-200'} text-gray-700 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all appearance-none`}
                                            >
                        ...
                        <div className="lg:col-span-4 space-y-4 sm:space-y-5">
                            ...
                            <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl p-3 sm:p-4 text-white shadow-lg mb-4">
                                <div className="flex justify-between items-start mb-4 sm:mb-6">
                                    <div className="text-xs text-gray-400 font-medium uppercase tracking-widest">Medical Card</div>
                                    <Activity className="text-blue-400" size={16} />
                                </div>
                                <div className="text-base sm:text-lg font-mono tracking-widest mb-1 break-all">{formData.CardNumber}</div>
                                <div className="text-xs text-gray-400 mb-3 sm:mb-4">Card Number</div>
                                <div className="flex justify-between text-xs font-mono">
                                    <div>
                                        <div className="text-gray-500 text-[10px] uppercase">Issued</div>
                                        <div className="text-sm">{formData.issue_date}</div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-gray-500 text-[10px] uppercase">Expires</div>
                ...
                <div className="bg-gray-50 px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-100 flex flex-col sm:flex-row justify-end gap-3 shrink-0">
                    <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                        <button
                            ...
                            className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-semibold bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-all active:scale-95 min-h-[42px] w-full sm:w-auto"
                        >
                            <X size={16} />
                            Cancel
                        </button>
                        <button
                            type="submit"
                            disabled={loading}
                            className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-200 transition-all active:scale-95 min-h-[42px] w-full sm:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {loading ? (
