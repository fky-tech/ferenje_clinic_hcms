    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={selectedTest ? `Findings: ${selectedTest.test_name}` : `Ultrasound Studies: ${request?.FirstName}`} size="lg">
            <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4 bg-primary-50 p-3 rounded-lg text-sm">
                    <div>
                        <p className="text-primary-700 font-semibold text-xs uppercase">Patient</p>
                        <p className="font-bold">{request?.FirstName} {request?.Father_Name} ({request?.Age} {request?.Sex})</p>
                    </div>
                    <div>
                        <p className="text-primary-700 font-semibold text-xs uppercase">Requested By</p>
                        <p className="font-bold">Dr. {request?.doctor_first_name} {request?.doctor_last_name}</p>
                    </div>
                </div>

                {loading ? <div className="text-center py-10 text-primary-600 font-medium">Loading details...</div> : (
                    <>
                        {!selectedTest ? (
                                ...
                                <div className="grid gap-3">
                                    {requestTests.map(test => (
                                        <div
                                            ...
                                            className="p-4 border border-primary-100 rounded-xl bg-white hover:bg-primary-50 cursor-pointer transition-all flex justify-between items-center shadow-sm"
                                        >
                                            ...
                                            <Button size="sm" variant="primary">Enter Results</Button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                                    ...
                                    {editingFindingIndex === null ? (
                                        <div className="space-y-4">
                                            ...
                                            <div className="grid gap-3">
                                                {findings.map((finding, idx) => (
                                                    ...
                                                    >
                                                        <div
                                                            ...
                                                            className="flex-1 p-4 border border-primary-100 rounded-xl bg-white hover:bg-primary-50 cursor-pointer transition-all flex justify-between items-center shadow-sm group"
                                                        >
                                                ...
                                                <button
                                                    ...
                                                    className="w-full flex items-center justify-center px-4 py-4 bg-white border-2 border-dashed border-primary-200 text-primary-600 rounded-xl hover:bg-primary-50 hover:border-primary-300 transition-all font-bold"
                                                >
                                        ...
                                        <div className="border border-blue-100 rounded-xl bg-white shadow-sm p-4 relative animate-in fade-in slide-in-from-bottom-2 duration-300">
                                            <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-2">
                                                <h4 className="font-bold text-primary-700 uppercase tracking-wider text-sm">
                                                    Editing Finding
                                                </h4>
                                            ...
                                            <div className="space-y-4">
                                                <div>
                                                    <label className="block text-xs font-bold text-primary-600 mb-1 uppercase tracking-wider">Title / Organ</label>
                                                    <input
                                                        type="text"
                                                        value={findings[editingFindingIndex].title}
                                                        onChange={(e) => handleTitleChange(editingFindingIndex, e.target.value)}
                                                        className="w-full px-4 py-2 bg-primary-50/30 border border-primary-100 rounded-lg focus:ring-2 focus:ring-primary-500 font-medium"
                                                ...
                                                <div>
                                                    <div className="flex justify-between items-center mb-2">
                                                        ...
                                                        <button
                                                            ...
                                                            className="flex items-center text-[10px] font-bold text-primary-600 hover:text-primary-800"
                                                        >
                                                    ...
                                                    <div className="space-y-2">
                                                        {findings[editingFindingIndex].descriptions.map((desc, dIndex) => (
                                                            <div key={dIndex} className="flex items-start gap-2 group/bullet">
                                                                <span className="mt-2.5 w-1.5 h-1.5 bg-primary-400 rounded-full flex-shrink-0" />
                                                                <textarea
                                                                    value={desc}
                                                                    onChange={(e) => handleDescriptionChange(editingFindingIndex, dIndex, e.target.value)}
                                                                    className="flex-1 px-3 py-1.5 bg-gray-50 border border-gray-100 rounded-lg focus:ring-1 focus:ring-primary-400 text-sm resize-none"
